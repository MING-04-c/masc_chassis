    ...
    def __import_type_support__(cls):
        try:
            ...
            logger = logging.getLogger(
                'masc_chassis_msgs.msg.RcStatus')
            logger.debug(
...
class RcStatus(metaclass=Metaclass_RcStatus):
    ...
    def __init__(self, **kwargs):
        ...
        if 'channels' not in kwargs:
            self.channels = numpy.zeros(7, dtype=numpy.int8)
        else:
        ...
        if self._check_fields:
            if isinstance(value, numpy.ndarray):
                assert value.dtype == numpy.int8, \
                    "The 'channels' numpy.ndarray() must have the dtype of 'numpy.int8'"
                assert value.size == 7, \
                    "The 'channels' numpy.ndarray() must have a size of 7"
            ...
            assert \
                ...
                "The 'channels' field must be a set or sequence with length 7 and each value of type 'int' and each integer in [-128, 127]"
        self._channels = numpy.array(value, dtype=numpy.int8)
